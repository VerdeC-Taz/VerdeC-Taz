...
{
	int median;
	//arrange density values in ascending order before finding median
	for(int i=0;i<countryCount-1;i++ )//the shifting will happen one less the number of counts
	{
		for(int j=0;j<countryCount-1-i;j++)
		{
			if(getdensity(j)>getdensity(j+1))
			{
				int jump=getdensity(j);
				setdens(getdensity(j+1));
				setdens(jump);
			}	
		}
	}
    median=getdensity(countryCount/2);//finding median
